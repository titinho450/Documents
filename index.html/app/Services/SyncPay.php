...
class SyncPay
{
    ...
    public function cashIn(array $payload): array
    {
        ...
        try {
            ...
            Log::debug('SyncPay API response:', ['response' => $response]);
    ...
    public function cashOut(float $amount, string $beneficiaryName, string $beneficiaryDocument, string $pixKey, string $pixType): array
    {
        ...
        try {
            ...
            if (!($response['data'] ?? null)) {
                if ($response['message API']) {
                    \Log::error('Erro ao processar pagamento:', $response);
                    throw new SyncPayException('Erro ao processar saque: ' . $response['message API']);
                } elseif ($response['message']) {
                    \Log::error('Erro ao processar pagamento:', $response);
                    throw new SyncPayException('Erro ao processar saque: ' . $response['message']);
                } else {
    ...
    private function makeRequest(string $method, string $endpoint, array $data = [], $headers = []): array
    {
        try {
            ...
            Log::debug('SyncPay Resposta Body:', ['response' => $responseBody]);

            // Decodifica o JSON
            $decodedResponse = json_decode($responseBody, true);
            Log::debug('SyncPay Resposta Decodificada:', ['response' => $decodedResponse]);

            return $decodedResponse;
        } catch (ClientException $e) {
            // Erros 5xx
            Log::error('SyncPay Erro Servidor:', ['message' => $e->getMessage(), 'payload' => $data]);
            ...
            // Erros 5xx
            Log::error('SyncPay Erro se Servidor:', ['message' => $e->getMessage()]);
            throw new SyncPayException(
                'Erro do servidor: ' . $e->getMessage() . 'Payload: ' . json_encode($data),
                $e->getCode()
            );
            ...
            Log::error('SyncPay Erro Requisição:', ['message' => $e->getMessage()]);
            throw new SyncPayException(
            ...
            Log::error('SyncPay Erro Geral:', ['message' => $e->getMessage()]);
            throw new SyncPayException(
